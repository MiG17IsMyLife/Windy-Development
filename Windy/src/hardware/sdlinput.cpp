/**
 * @file sdlinput.cpp
 * @brief SDL Input System implementation
 * C++ port of lindbergh-loader's sdlInput.c
 */
...
int SdlInput::Init(const char *controlsPath)
{
    ...
    m_initialized = true;
    log_info("SdlInput initialized (gameType=%d, analogMax=%d, controllers=%d)", (int)gameType, m_jvsAnalogueMaxValue,
             m_controllers.joysticksCount);
    return 0;
}
...
void SdlInput::SetDefaultMappings()
{
    ...
    if (gt == DIGITAL)
    {
        bindings = g_defaultDigitalBindings;
        count = g_defaultDigitalBindingsSize;
    }
    else if (gt == DRIVING)
    {
        bindings = g_defaultDrivingBindings;
        count = g_defaultDrivingBindingsSize;
    }
    else if (gt == GAME_TYPE_ABC)
    {
        bindings = g_defaultAbcBindings;
        count = g_defaultAbcBindingsSize;
    }
    else if (gt == SHOOTING)
    {
        bindings = g_defaultShootingBindings;
        count = g_defaultShootingBindingsSize;
    }
    else if (gt == MAHJONG)
    {
        bindings = g_defaultMahjongBindings;
        count = g_defaultMahjongBindingsSize;
    }
...
int SdlInput::GetControllerID(SDL_JoystickID instanceId)
{
    for (int i = 0; i < MAX_JOYSTICKS; i++)
        if (m_controllers.controllers[i])
        {
            ...
            if (joy && SDL_GetJoystickID(joy) == instanceId)
                return i;
        }
    return -1;
}
...
int SdlInput::GetJoystickID(SDL_JoystickID instanceId)
{
    for (int i = 0; i < MAX_JOYSTICKS; i++)
        if (m_controllers.joysticks[i])
            if (SDL_GetJoystickID(m_controllers.joysticks[i]) == instanceId)
                return i;
    return -1;
}
    ...
    switch (e->type)
    {
            ...
            if (devIdx != -1 && e->jaxis.axis < MAX_JOY_AXES)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (fabsf(state->positiveContribution - newVal) > 0.001f)
                            {
                        ...
                        {
                            if (fabsf(state->negativeContribution - newVal) > 0.001f)
                            {
                        ...
                        {
                            if (fabsf(state->analogValue - newVal) > 0.001f)
                            {
        ...
        case SDL_EVENT_JOYSTICK_HAT_MOTION:
        {
            ...
            if (devIdx != -1 && e->jhat.hat < MAX_JOY_HATS)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (dirs[i]->type == INPUT_TYPE_NONE)
                        continue;
                    ...
                    {
                        state->isActive = active;
                        AddActionToDirtyList(dirs[i]->player, dirs[i]->action);
                    }
                }
            }
            break;
        ...
        case SDL_EVENT_MOUSE_MOTION:
        {
            ...
            if (m_dest.W > 0 && m_dest.H > 0)
            {
                if (mX <= m_dest.X)
                    posX = 0;
                else if (mX >= (m_dest.W + m_dest.X - 5))
                    posX = (float)m_dest.W;
                else
                    posX = mX - m_dest.X;
                ...
                if (mY <= m_dest.Y)
                    posY = 0;
                else if (mY >= (m_dest.H + m_dest.Y - 5))
                    posY = (float)m_dest.H;
                else
                    posY = mY - m_dest.Y;
            ...
            if (gt == SHOOTING && (posX <= 0.01f || posX >= 0.99f || posY <= 0.01f || posY >= 0.99f))
            {
                if (gid == RAMBO || gid == RAMBO_CHINA || gid == TOO_SPICY)
                {
                    ...
                    if (bx->type != INPUT_TYPE_NONE)
                    {
                        m_actionStates[bx->player][LA_GunX].analogValue = -1;
                        AddActionToDirtyList(bx->player, LA_GunX);
                    }
                    if (by->type != INPUT_TYPE_NONE)
                    {
                        m_actionStates[by->player][LA_GunY].analogValue = -1;
                        AddActionToDirtyList(by->player, LA_GunY);
                    }
                }
            ...
            if (bindX->type != INPUT_TYPE_NONE)
            {
                ...
                if (fabsf(st->analogValue - val) > 0.001f)
                {
                    st->analogValue = val;
                    AddActionToDirtyList(bindX->player, bindX->action);
                }
            }
            if (bindY->type != INPUT_TYPE_NONE)
            {
                ...
                {
                    st->analogValue = val;
                    AddActionToDirtyList(bindY->player, bindY->action);
                }
            }
            break;
        }
...
void SdlInput::UpdateGunShake()
{
    ...
    for (int p = PLAYER_1; p <= PLAYER_2; p++)
    {
        ...
        if (dirX != 0 && dirX == -m_lastGunXDir[p])
            m_shakeValue[p] += abs(dx) * m_shakeIncreaseRate;
        if (dirY != 0 && dirY == -m_lastGunYDir[p])
            m_shakeValue[p] += abs(dy) * m_shakeIncreaseRate;

        m_shakeValue[p] *= m_shakeDecayRate;
        if (m_shakeValue[p] < 1.0f)
            m_shakeValue[p] = 0.0f;
        if (m_shakeValue[p] > 512.0f)
            m_shakeValue[p] = 512.0f;

        int jvsOut = 512 + (int)m_shakeValue[p];
        if (p == PLAYER_1)
        {
            jvs->SetAnalogue(ANALOGUE_5, jvsOut);
            jvs->SetAnalogue(ANALOGUE_6, jvsOut);
        }
        else if (p == PLAYER_2)
        {
            jvs->SetAnalogue((JVSInput)ANALOGUE_7, jvsOut);
            jvs->SetAnalogue((JVSInput)ANALOGUE_8, jvsOut);
        }

        m_lastGunX[p] = curX;
        m_lastGunY[p] = curY;
        if (dirX != 0)
            m_lastGunXDir[p] = dirX;
        if (dirY != 0)
            m_lastGunYDir[p] = dirY;
    }
}
...
// DetectCombinedAxes
// ============================================================
void SdlInput::ScanAxisBindings(BindingPair *pair, bool hasFullAxis[], bool hasHalfAxis[], bool hasPositive[][NUM_LOGICAL_ACTIONS],
                                bool hasNegative[][NUM_LOGICAL_ACTIONS])
{
    for (int k = 0; k < 2; k++)
    {
        ControlBinding *b = &pair->bindings[k];
        if (b->type == INPUT_TYPE_NONE)
            continue;
        int act = b->action;
        int p = b->player;
        if (b->axisMode == AXIS_MODE_FULL)
            hasFullAxis[act] = true;
        if (b->axisMode == AXIS_MODE_POSITIVE_HALF)
        {
            hasHalfAxis[act] = true;
            hasPositive[p][act] = true;
        }
        if (b->axisMode == AXIS_MODE_NEGATIVE_HALF)
        {
            hasHalfAxis[act] = true;
            hasNegative[p][act] = true;
        }
    }
}
...
void SdlInput::ParseSdlSource(const char *token, JVSPlayer player, LogicalAction action)
{
    ...
    if (strncmp(buf, "KEY_", 4) == 0)
    {
        ...
        if (_stricmp(buf + 4, "Comma") == 0)
            map.sdlId = SDL_SCANCODE_COMMA;
        else
            map.sdlId = SDL_GetScancodeFromName(buf + 4);
    }
    else if (strncmp(buf, "GC", 2) == 0)
    {
        ...
        if (sscanf(buf, "GC%d_%31[^_]_%63s", &devId, typeStr, nameStr) == 3)
        {
            ...
            else if (strcmp(typeStr, "AXIS") == 0)
            {
                ...
                if (strstr(buf, "_POSITIVE_HALF"))
                    map.axisMode = AXIS_MODE_POSITIVE_HALF;
                else if (strstr(buf, "_NEGATIVE_HALF"))
                    map.axisMode = AXIS_MODE_NEGATIVE_HALF;
                else if (strstr(buf, "_POSITIVE"))
                {
                    map.axisMode = AXIS_MODE_DIGITAL;
                    map.axisThreshold = 1;
                }
                else if (strstr(buf, "_NEGATIVE"))
                {
                    map.axisMode = AXIS_MODE_DIGITAL;
                    map.axisThreshold = -1;
                }
    ...
    {
        const char *mt = buf + 6;
        if (strcmp(mt, "AXIS_X") == 0)
        {
            map.type = INPUT_TYPE_MOUSE_AXIS;
            map.sdlId = 0;
        }
        else if (strcmp(mt, "AXIS_Y") == 0)
        {
            map.type = INPUT_TYPE_MOUSE_AXIS;
            map.sdlId = 1;
        }
        else if (strcmp(mt, "LEFT_BUTTON") == 0)
        {
            map.type = INPUT_TYPE_MOUSE_BUTTON;
            map.sdlId = SDL_BUTTON_LEFT;
        }
        else if (strcmp(mt, "RIGHT_BUTTON") == 0)
        {
            map.type = INPUT_TYPE_MOUSE_BUTTON;
            map.sdlId = SDL_BUTTON_RIGHT;
        }
        else if (strcmp(mt, "MIDDLE_BUTTON") == 0)
        {
            map.type = INPUT_TYPE_MOUSE_BUTTON;
...
bool SdlInput::LoadProfileFromIni(const char *controlsPath)
{
    ...
    if (common)
    {
        for (const auto &pair : common->pairs)
        {
            ...
            if (!ParseActionKey(pair.key.c_str(), &player, &action))
                continue;
            if (player != SYSTEM)
            ...
            while (tok)
            {
                ParseSdlSource(tok, player, action);
                tok = strtok(nullptr, ", ");
            }
            free(dup);
        }

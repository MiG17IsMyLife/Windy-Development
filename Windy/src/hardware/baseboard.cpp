...
int BaseBoard::Ioctl(unsigned int request, void* data) {
    ...
    case BASEBOARD_READ_SRAM:
    {
        ...
        if (m_sram && _data) {
            ...
            log_debug("BaseBoard: BASEBOARD_READ_SRAM offset=%u size=%u",
                _data->offset, _data->size);
        }
        return 0;
    }
    ...
    case BASEBOARD_WRITE_SRAM:
    {
        ...
        if (m_sram && _data) {
            ...
            log_debug("BaseBoard: BASEBOARD_WRITE_SRAM offset=%u size=%u",
                _data->offset, _data->size);
        }
        return 0;
    }
...
void BaseBoard::ProcessRequest(uint32_t* data) {
    ...
    case BASEBOARD_GET_SERIAL:  // bcCmdSysInfoGetReq (0x120)
    {
        ...
        log_debug("BaseBoard: REQUEST GET_SERIAL destAddr=%u destSize=%u",
            m_serialCommand.destAddress, m_serialCommand.destSize);
    }
    break;
    ...
    case BASEBOARD_PROCESS_JVS:  // 0x220
    {
        ...
        log_trace("BaseBoard: REQUEST PROCESS_JVS src=%u/%u dest=%u/%u",
            m_jvsCommand.srcAddress, m_jvsCommand.srcSize,
        ...
        if (m_jvsBoard && m_jvsCommand.srcSize > 0) {
            ...
            size_t copySize = (m_jvsCommand.srcSize < sizeof(inputBuffer)) ?
                m_jvsCommand.srcSize : sizeof(inputBuffer);
            memcpy(inputBuffer, &m_sharedMemory[m_jvsCommand.srcAddress], copySize);

            // Process JVS packet
            int outputSize = 0;
            m_jvsBoard->ProcessPacket(inputBuffer, (unsigned int)copySize,
...
void BaseBoard::ProcessReceive(uint32_t* data) {
    ...
    case BASEBOARD_GET_SERIAL:  // 0x120
    {
        ...
        memcpy(&m_sharedMemory[m_serialCommand.destAddress + 96],
            m_serialString, strlen(m_serialString));
    ...
    case BASEBOARD_PROCESS_JVS:  // 0x220
    {
        ...
        log_trace("BaseBoard: RECEIVE PROCESS_JVS -> addr=%u size=%d",
            m_jvsCommand.destAddress, m_jvsPacketSize);
    }
    break;

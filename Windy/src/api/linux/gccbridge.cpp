...
extern "C"
{
    ...
    static uint64_t _udivmoddi4(uint64_t num, uint64_t den, uint64_t *rem)
    {
        if (den == 0)
        {
            // Trigger divide by zero crash
            volatile int x = 0;
            return 1 / x;
        }
    ...
    int64_t __divdi3(int64_t a, int64_t b)
    {
        ...
        int64_t res = _udivmoddi4(a, b, NULL);
        if (neg)
            res = -res;
        return res;
    }
